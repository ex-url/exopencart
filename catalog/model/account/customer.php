...
class ModelAccountCustomer extends Model {
	...
	public function editAffiliate($customer_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "customer_affiliate SET `company` = '" . $this->db->escape($data['company']) . "', `website` = '" . $this->db->escape($data['website']) . "', `commission` = '" . (float)$this->config->get('config_affiliate_commission') . "', `tax` = '" . $this->db->escape($data['tax']) . "', `payment_details` = '" . $this->db->escape($data['payment_details']) . "' WHERE `customer_id` = '" . (int)$customer_id . "'");
	}
	...
	public function getTotalOrdersByTracking($tracking) {
		$query = $this->db->query("SELECT COUNT(*) as total FROM " . DB_PREFIX . "order WHERE tracking = '" . $this->db->escape($tracking) . "' AND order_status_id > 0");
		return (int)$query->row['total'];
	}

	public function updateAffiliateClicks($customer_id) {
		$this->db->query("UPDATE `" . DB_PREFIX . "customer_affiliate` SET clicks = (clicks + 1) WHERE `customer_id` = '" . (int)$customer_id . "'");
	}
}
