...
class ControllerBlogArticle extends Controller {
	...
	public function index() {
		...
		if ($article_info) {
			...
			foreach ($results as $result) {
				$data['images'][] = array(
					'popup' => $this->model_tool_image->resize($result['image'], $this->config->get('configblog_image_article_width'), $this->config->get('configblog_image_article_height')),
			...
			foreach ($results as $result) {
				if (file_exists(DIR_DOWNLOAD . $result['filename'])) {
					...
					$data['downloads'][] = array(
						...
						'href'       => $this->url->link('blog/article/download', '&source_id='. $this->request->get['article_id']. '&download_id=' . $result['download_id'])
					);
				}
			}
	...
	public function download() {
		...
		if (isset($this->request->get['source_id'])) {
			$article_id = $this->request->get['source_id'];
		} else {
			$article_id = 0;
		}
