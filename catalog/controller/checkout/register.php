...
class ControllerCheckoutRegister extends Controller {
	...
	public function save() {
		...
		if ($this->customer->isLogged()) {
			$json['redirect_register_save'] = $this->url->link('checkout/checkout', '', true);
		}

		// Validate cart has products and has stock.
		if ((!$this->cart->hasProducts() && empty($this->session->data['vouchers'])) || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
			$json['redirect_register_save'] = $this->url->link('checkout/cart');
		}
		...
		if (!$json) {
			...
			if ($customer_group_info && !$customer_group_info['approval']) {
				...
				//
				// if($this->model_account_address->getAddress($this->customer->getAddressId())) {
				// 	$this->session->data['payment_address'] = $this->model_account_address->getAddress($this->customer->getAddressId());
				// }
				//
				// if (!empty($this->request->post['shipping_address'])) {
				// 	$this->session->data['shipping_address'] = $this->session->data['payment_address'] = $this->model_account_address->getAddress($this->customer->getAddressId());
				// }
			} else {
