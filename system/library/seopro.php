...
class SeoPro {
  ...
  public function baseRewrite($data, $language_id) {
    ...
    $use_flat_category = false;
    $use_flat_product = false;
    ...
    switch ($data['route']) {
      case 'product/product':
        if (isset($data['product_id'])) {
          ...
          if($this->config->get('config_seo_url_flat_structure')) {
            $use_flat_product = true;
          }
    ...
    if (empty($queries) && $route) {
      ...
      if (!empty($rows) && (count($rows) == count($queries))) {
        if ($use_flat_category) {
          $url .= '/' . rawurlencode($category_prefix);
        }
        
        if ($use_flat_product) {
          $url .= '/' . rawurlencode($product_prefix);
        }
